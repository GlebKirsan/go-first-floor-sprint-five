...
const (
	...
	Eps        = 1e-4 // погрешность для сравнения float с нулём
)
...
func nearlyZero(number float64) bool {
	return math.Abs(number) < Eps
}

// meanSpeed возвращает среднюю скорость бега или ходьбы.
func (t Training) meanSpeed() float64 {
	if nearlyZero(t.Duration.Hours()) {
		return 0
	}
...
func (s Swimming) meanSpeed() float64 {
	if nearlyZero(s.Duration.Hours()) {
		return 0
	}
